...
@pytest.mark.asyncio
async def test_list_meetings_empty(async_client):
...
@pytest.mark.asyncio
async def test_list_meetings_with_data(async_client, seeded_meeting):
    resp = await async_client.get("/meetings")
    assert resp.status_code == 200
    meetings = resp.json()
...
@pytest.mark.asyncio
async def test_get_meeting_detail(async_client, seeded_meeting, seeded_action_item, seeded_participant):
    resp = await async_client.get(f"/meetings/{seeded_meeting.id}")
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.asyncio
async def test_get_meeting_detail_not_found(async_client):
    resp = await async_client.get("/meetings/nonexistent-id")
...
@pytest.mark.asyncio
async def test_update_action_item_status(async_client, seeded_meeting, seeded_action_item):
...
@pytest.mark.asyncio
async def test_update_action_item_wrong_meeting(async_client, seeded_meeting, seeded_action_item):
...
@pytest.mark.asyncio
async def test_update_participant_email(async_client, seeded_meeting, seeded_participant):
    resp = await async_client.patch(
...
@pytest.mark.asyncio
async def test_update_participant_email_not_found(async_client, seeded_meeting):
    resp = await async_client.patch(
...
@pytest.mark.asyncio
async def test_delete_meeting(async_client, seeded_meeting):
    resp = await async_client.delete(f"/meetings/{seeded_meeting.id}")
    assert resp.status_code == 200
    assert resp.json()["deleted"] is True

    # Confirm it's gone
    resp2 = await async_client.get(f"/meetings/{seeded_meeting.id}")
...
@pytest.mark.asyncio
async def test_send_slack_success(async_client, seeded_meeting, seeded_action_item, seeded_participant):
    with patch(
        "api.routes.send_slack_for_meeting",
        new=AsyncMock(return_value={"success": True, "error": None}),
    ):
        resp = await async_client.post(f"/meetings/{seeded_meeting.id}/send/slack")
    assert resp.status_code == 200
    assert resp.json()["sent"] == 1


@pytest.mark.asyncio
async def test_send_slack_tool_failure(async_client, seeded_meeting):
    with patch(
        "api.routes.send_slack_for_meeting",
...
@pytest.mark.asyncio
async def test_send_slack_meeting_not_found(async_client):
    resp = await async_client.post("/meetings/bad-id/send/slack")
    assert resp.status_code == 404
...
@pytest.mark.asyncio
async def test_send_jira_success(async_client, seeded_meeting, seeded_action_item):
    with patch(
        "api.routes.send_jira_for_meeting",
        new=AsyncMock(return_value={"created": ["PROJ-1"], "failed": []}),
    ):
        resp = await async_client.post(f"/meetings/{seeded_meeting.id}/send/jira")
    assert resp.status_code == 200
    data = resp.json()
    assert data["sent"] == 1
...
@pytest.mark.asyncio
async def test_send_jira_no_action_items(async_client, seeded_meeting):
...
@pytest.mark.asyncio
async def test_send_jira_credentials_missing(async_client, seeded_meeting, seeded_action_item):
    with patch(
        "api.routes.send_jira_for_meeting",
...
@pytest.mark.asyncio
async def test_send_email_no_emails_configured(async_client, seeded_meeting, seeded_action_item):
    """If no participant has an email, return a helpful message (not error)."""
    # The seeded participant has no email in this fixture variant.
    resp = await async_client.post(f"/meetings/{seeded_meeting.id}/send/email")
    # Expected: 200 with message about missing emails (participant has no email in this session)
    assert resp.status_code == 200


@pytest.mark.asyncio
async def test_send_email_with_participant_email(async_client, seeded_meeting, seeded_action_item, seeded_participant):
    with patch(
...
@pytest.mark.asyncio
async def test_send_calendar_success(async_client, seeded_meeting, seeded_participant):
    with patch(
...
@pytest.mark.asyncio
async def test_send_calendar_tool_error(async_client, seeded_meeting, seeded_participant):
    with patch(
        "api.routes.send_calendar_for_meeting",
...
async def test_get_job_status_not_found(async_client):
    resp = await async_client.get("/meetings/status/nonexistent-job-id")
    assert resp.status_code == 404
...
@pytest.mark.asyncio
async def test_query_no_meetings(async_client):
    resp = await async_client.post("/query", json={"question": "What were the decisions?"})
    assert resp.status_code == 200
    assert "No meetings are available" in resp.json()["answer"]


@pytest.mark.asyncio
async def test_query_participants(async_client, seeded_meeting, seeded_participant):
    resp = await async_client.post(
        "/query",
...
@pytest.mark.asyncio
async def test_query_action_items(async_client, seeded_meeting, seeded_action_item):
    resp = await async_client.post(
        "/query",
...
@pytest.mark.asyncio
async def test_upload_unsupported_file_type(async_client):
    import io
    resp = await async_client.post(
        "/meeting/upload",
